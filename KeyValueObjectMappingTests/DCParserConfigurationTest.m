...
- (void) testCreateCustomTweetWithBlock {
    DCCustomInitializeBlock block = ^id(__weak Class classToGenerate, NSDictionary *values, id parentObject){
        XCTAssertEqual(classToGenerate, [Tweet class], @"classToGenerate should be an tweet");
    ...
    XCTAssertTrue([tweet.text isEqualToString: @"Should be an text"], @"should have the same text passed on custom initialize");
}
...
- (void) testCreateObjectWithNormalInitialize {
    ...
    Tweet *tweet = [configuration instantiateObjectForClass:[Tweet class] withValues:nil];
    XCTAssertNotNil(tweet, @"should create an tweet when no custom initialize is used");
}

- (void) testWrongSplitTocken {
    DCParserConfiguration *configuration = [DCParserConfiguration configuration];
    configuration.splitToken = @".";
    XCTAssertFalse([configuration.splitToken isEqualToString:@"."], @"splitToken value should not have change");
    
    configuration.nestedPrepertiesSplitToken = @"|";
    configuration.splitToken = @".";
    XCTAssertTrue([configuration.splitToken isEqualToString:@"."], @"splitToken value should have change");
}
